...
fn pad_message(message: &[u8]) -> Vec<u8> {
    ...
    // Додаємо одиницю '1' у бітовому представленні
    padded.push(0x80);

    // Додаємо '0' біти до тих пір, поки довжина повідомлення в бітах ≡ 448 (mod 512)
    while (padded.len() % BLOCK_SIZE) != (BLOCK_SIZE - 8) {
        padded.push(0);
    }

    // Додаємо початкову довжину повідомлення у бітах, у вигляді 64-бітного цілого числа в порядку big-endian
...
fn hash_sha1(message: &[u8]) -> [u8; 20] {
    ...
    for i in 0..block_count {
        ...
        // Ініціалізуємо перші 16 слів даними блоку
        for j in 0..16 {
            let offset = j * 4;
